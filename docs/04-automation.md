...
7. 创建仓库级 Variable：`NOTIFICATIONS_ENABLED`，初始值设为 `false`
8. 确认 Actions 已启用
9. 手动触发一次 `.github/workflows/investment_bot.yml`，保持 `send_notification=false` 做干跑
10. 飞书机器人可用后，再用 `send_notification=true` 验证真实通知
11. 验证成功后把 `NOTIFICATIONS_ENABLED` 改成 `true`，开启每日定时通知

当前工作流会：

- 从 `secrets.FEISHU_WEBHOOK_URL` 读取 webhook
- 从 `secrets.PORTFOLIO_JSON` 和 `secrets.STRATEGY_JSON` 注入真实配置
- 通过 GitHub Actions cache 恢复上次状态
- 每次先执行类型检查和测试
- 手动运行默认只干跑，可选择真实发送或强制输出调仓建议
- 定时运行只在 `NOTIFICATIONS_ENABLED=true` 时执行
- 运行结束后把新的 `state.json` 自动保存到 cache
...
所以本地和 GitHub 使用同一份状态格式，只是载体不同：

- 本地更适合编辑忽略文件
- GitHub 通过 Secret 注入配置，通过 Actions cache 持久化状态
