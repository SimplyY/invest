...
8. 创建仓库级 Variable：`NOTIFICATIONS_ENABLED`，初始值设为 `false`
9. 确认 Actions 已启用
10. 手动触发一次 `.github/workflows/investment_bot.yml`，保持 `send_notification=false` 做干跑
11. 飞书机器人可用后，再用 `send_notification=true` 验证真实通知
12. 验证成功后把 `NOTIFICATIONS_ENABLED` 改成 `true`，开启每日定时通知
